...
def init_db():
    ...
    # ── Migrate: strip accidental printf-style format specs from question text
    import re as _re
    _fmt_pat = _re.compile(r'%[sdrfieoxXucp]')
    for _tbl, _col in [("questions","question"),("questions","option_a"),("questions","option_b"),
                       ("questions","option_c"),("questions","option_d")]:
        _rows = conn.execute(f"SELECT id, {_col} FROM {_tbl}").fetchall()
        for _row in _rows:
            _val = _row[1] or ""
            if _fmt_pat.search(_val):
                _fixed = _fmt_pat.sub("", _val).strip()
                conn.execute(f"UPDATE {_tbl} SET {_col}=? WHERE id=?", (_fixed, _row[0]))
    conn.commit()
...
def get_categories():
    db = get_db()
    rows = db.execute("SELECT c.id,c.name,c.icon,c.color,COUNT(q.id) AS question_count FROM categories c LEFT JOIN questions q ON c.id=q.category_id GROUP BY c.id ORDER BY c.id").fetchall()
